...
本手册按照**工程化方式**组织，可作为：**项目即插即用 + Prompt 即粘即用 + 模板即拷即用** 的速成指南。
...
**标题**：Power BI / Fabric / AI 驱动的语义层与数据产品工程师

**核心卖点**：
- 用 AI 生成 DAX/M/模型方案，30 分钟交付可运行样板
- 语义层治理（Base/Logic/Report 度量分层、指标口径统一）
- Dev→Test→Prod 部署流水线 + 脚本化治理
- RLS/数据血缘/认证数据集/移动端/领导驾驶舱
- 制造场景（OEE/停机/良率）闭环

**项目经历（STAR）**：问题 → 设计 → AI 生成 → 性能/安全 → 结果

### 18.2 简历段落样例

```text
在制造工厂场景中，构建基于星型模型的语义层，使用 AI 自动生成 DAX/M 与度量分层，
将周度 KPI 报表的交付周期从 3 天缩短至 0.5 天；引入部署流水线与动态 RLS，实现 Dev→Prod 的
自动化发布与权限治理；通过性能分析器与 Query Folding 优化，将慢查询 P95 从 11.3s 降至 1.8s。
```

---

## 十九、附录 A：AI Prompt 词库

### A.1 模型设计

```text
你是 Power BI/Fabric 语义层架构师。根据以下业务事件与字段，产出星型模型方案：
- 表清单、主键/外键、方向/活跃性
- 角色扮演维度策略
- 多对多替代方案
- 模型风险与测试清单
...
```text
请根据规则：{中文业务规则列表}
输出完整 M 脚本，逐步注释，保持可折叠；并提供不可折叠步骤的源端 SQL 等价实现。
```

### A.3 DAX 度量

```text
请基于 Base 度量清单，生成 Logic 层与 Report 层度量：
- 防空、上下文说明
- 动态切换（SWITCH/SELECTEDVALUE）
- TopN + Others 聚合
- 格式化（百分比/小数位）
```
...
```text
请生成动态 RLS 规则：UserEmail 基于 USERPRINCIPALNAME() 匹配，
限制到 Plant/Line；给出测试方法与注意事项（Import/DirectQuery）。
```

### A.5 性能体检

```text
输入：模型图、行数、慢度量、性能分析器输出。
输出：数据层/模型层/DAX/可视层的改进项、预期收益、验证步骤。
```

### A.6 Service 交付

```text
请输出工作区/应用的组织方案、权限矩阵、刷新计划、移动端固定策略、
以及与 Power Apps/Automate 的闭环流程。
```

### A.7 Cmdlets/REST 自动化

```text
请生成 PowerShell/REST 示例，用于：
- 列出工作区与成员
- 批量授权
- 导出报表/数据集清单
- 触发刷新并查询状态
...
> **注意**：以下 DAX 均由 AI 生成，你只"校对口径"。

### B.1 产量/达成率

```DAX
Actual = SUM(FactProduction[ActualQty])
Plan   = SUM(FactProduction[PlanQty])
AchRate = DIVIDE([Actual], [Plan], 0)
```

### B.2 停机 TopN + Others

```DAX
Downtime = SUM(FactLoss[DowntimeMin])

TopN Loss Table =
VAR _Top =
    TOPN(
        5,
        SUMMARIZE(FactLoss, FactLoss[LossReason], "Min", [Downtime]),
        [Min], DESC
    )
VAR _Others =
    ROW("LossReason", "Others", "Min",
        [Downtime] - CALCULATE([Downtime], KEEPFILTERS(_Top))
    )
RETURN
UNION(_Top, _Others)
```
...
```DAX
DefectRate = DIVIDE(SUM(FactQuality[DefectQty]), SUM(FactQuality[TotalQty]), 0)
```
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| **度量出错** | 上下文不对 | 用 SELECTEDVALUE() 检查切片器 |
| **结果不稳定** | 双向关系误用 | 改回单向、用桥接表 |
| **刷新失败** | Gateway 凭据变化 | 在 Service 重新映射 |
| **性能慢** | 视觉太多交互 | 减少交互、优化 DAX |
| **RLS 不生效** | 大小写不一致 | 检查 USERPRINCIPALNAME |
| **DirectQuery 限制** | 下推失败 | 考虑 Hybrid 方案 |
...
1. **拿实际制造数据** → 按附录 B 度量模板跑起来（30–60 分钟）
2. **把业务口径写成中文** → 用附录 A prompt 生成 DAX/M/模型方案
3. **跑性能与权限** → 用附录 A 的体检/RLS prompt
4. **发布与治理** → Service + Pipeline + Cmdlets/REST
5. **写入 Marketing CV** 
